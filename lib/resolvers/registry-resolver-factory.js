    ...
    resolve: function (fqn, model) {
      return resolveTdef(fqn)
        .then(function (regTdef) {
          ...
          logger.info('KevScript', 'Found ' + fqn + ' in ' + registryUrl());
        ...
        .then(function (data) {
          ...
          if (data.dus.length === 0) {
            throw new Error('No DeployUnit found for ' + fqn.namespace + '.' + fqn.name + '/' + fqn.version.tdef + ' that matches ' + JSON.stringify(fqn.version.du));
          }
