...
module.exports = function (model, statements, stmt, opts, cb) {
	var nameList;
	if (!stmt.instances) { stmt.instances = {}; }
        ...
        nameList.forEach(function (instancePath) {
          ...
          if (instancePath.length === 1) {
            ...
            if (tdef.metaClassName() === 'org.kevoree.NodeType') {
              instance = model.findNodesByID(instancePath[0].value) || factory.createContainerNode();
              ...
              instance = model.findGroupsByID(instancePath[0].value) || factory.createGroup();
              instance.name = instancePath[0].value;
              instance.started = true;
              instance.typeDefinition = tdef;
              inflateDictionary(instance);
              ...
              instance = model.findHubsByID(instancePath[0].value) || factory.createChannel();
              instance.name = instancePath[0].value;
              instance.started = true;
              instance.typeDefinition = tdef;
              inflateDictionary(instance);
            ...
            if (tdef.metaClassName() === 'org.kevoree.NodeType') {
              if (instancePath[0].value === '*') {
                ...
                if (hostNode) {
                  instance = model.findNodesByID(instancePath[1].value) || factory.createContainerNode();
                ...
                model.nodes.array.forEach(function (node) {
                  if (!node.host) {
                    if (modelHelper.isCompatible(tdef, node)) {
                      instance = node.findComponentsByID(instancePath[1].value) || factory.createComponentInstance();
                      instance.name = instancePath[1].value;
                      instance.started = true;
                      instance.typeDefinition = tdef;
                      inflateDictionary(instance);
                      ...
                      throw new KevScriptError('Unable to add \'' + instancePath[1].value + '\' (' + tdef.name + '/' + tdef.version + ') to \'' + node.name + '\' ('+node.typeDefinition.name+'/'+node.typeDefinition.version+'), no available \''+modelHelper.getPlatforms(node.typeDefinition)[0]+ '\' platform', instancePath[1].pos);
                    }
                  }
                });
              } else {
