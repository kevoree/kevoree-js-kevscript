                ...
                model.nodes.array.forEach(function (node) {
                  if (!node.host) {
                    if (modelHelper.isCompatible(tdef, node)) {
                      ...
                      throw new KevScriptError('Unable to add \'' + instancePath[1].value + '\' (' + tdef.name + '/' + tdef.version + ') to \'' + node.name + '\' ('+node.typeDefinition.name+'/'+node.typeDefinition.version+'), no available \''+modelHelper.getPlatforms(node.typeDefinition)[0]+ '\' platform');
                    }
                  }
                });
              } else {
