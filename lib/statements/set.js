...
module.exports = function (model, expressions, stmt, opts) {
	...
	// 		var instances;
	// 		if (res[0].length === 3) {
	// 			throw new KevScriptError('Setting fragmented attribute only makes sense for groups & channels. "' + res[0].value + '/' + res[1].value + '" can only refer to a component attribute. Set failed', res[0].pos);
	// 		} else if (res[0].length === 2) {
	// 			if (res[1].length === 1) {
	// 				instances = model
	// 					.select('/groups[' + res[0][0].value + ']').array
	// 					.concat(model.select('/hubs[' + res[0][0].value + ']').array);
	//
	// 				if (instances.length === 0 && res[0][0].value !== '*') {
	// 					throw new KevScriptError('There is no group or channel instance named "' + res[0][0].value + '". Set failed', res[0][0].pos);
	// 				}
	//
	// 				instances.forEach(function (instance) {
	// 					if (res[1][0] === '*') {
	...
	// 							updateDictionary(fDic, instance, res[0][1], res[2].value, true);
	// 						});
	// 					} else {
	// 						// specific fragment
	// 						var fDic = instance.findFragmentDictionaryByID(res[1][0].value);
	// 						if (fDic) {
	// 							updateDictionary(fDic, instance, res[0][1], res[2].value, true);
	// 						} else {
	// 							throw new KevScriptError('Unable to find fragment "' + res[1][0].value + '" for instance "' + res[0][0].value + '". Set failed', res[1][0].pos);
	// 						}
	// 					}
	// 				});
	// 			} else {
	// 				throw new KevScriptError('Invalid fragment path "' + res[1].value + '". Fragment path must be a node name. Set failed', res[1].pos);
	// 			}
	// 		} else {
	// 			throw new KevScriptError('"' + res[0].value + '" is not a valid attribute path', res[0].pos);
	// 		}
	// 	});
	// }
};
