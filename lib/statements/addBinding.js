		...
		if (res[1].length === 1) {
			if (res[1][0].value === '*') {
				...
				if (chan) {
					...
					throw new KevScriptError('Unable to find chan instance "' + res[1][0].value + '". Bind failed', res[1][0].pos);
				}
			}
		} else {
			throw new KevScriptError('Bind target path is invalid (' + res[1].value + ')', res[1].pos);
		}
		...
		if (res[0].length === 3) {
			if (res[0][0].value === '*') {
				...
				if (node) {
					...
					throw new KevScriptError('Unable to find node instance "' + res[0][0].value + '". Bind failed', res[0][0].pos);
				}
			}
		} else {
			throw new KevScriptError('"' + res[0].value + '" is not a valid bind path for a port', res[0].pos);
		}
		...
		nodes.forEach(function (node) {
			if (res[0][1].value === '*') {
				...
				if (comp) {
					...
					throw new KevScriptError('Unable to find component instance "' + res[0][1].value + '" in node "' + res[0][0].value + '". Bind failed', res[0][1].pos);
				}
			}
		});
			...
			if (res[0][2].value === '*') {
				...
				if (port) {
					...
					if (port) {
						...
						throw new KevScriptError('Component "' + comp.name + '" in node "' + comp.eContainer().name + '" has no port named "' + res[0][2].value + '". Bind failed', res[0][2].pos);
					}
				}
			}
		});
